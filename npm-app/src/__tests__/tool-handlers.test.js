...
describe('handleCodeSearch', () => {
    ...
    beforeEach(async () => {
        ...
        await fs.promises.writeFile(path.join(testDataDir, 'test-content.js'), `// Test file for code search
...
`);
        await fs.promises.writeFile(path.join(testDataDir, 'another-file.ts'), `// Another test file
...
`);
    ...
    test('finds specific content in test file', async () => {
        ...
        const result = await handleCodeSearch(parameters, 'test-id');
        expect(mockGetProjectRoot).toHaveBeenCalled();
        expect(result[0].value.stdout).toContain('UNIQUE_SEARCH_STRING_12345');
        expect(result[0].value.stdout).toContain('test-content.js');
    });
    test('searches with case-insensitive flag', async () => {
        ...
        const result = await handleCodeSearch(parameters, 'test-id');
        expect(result[0].value.stdout).toContain('findme_xyz789');
    });
    test('limits results when maxResults is specified', async () => {
        ...
        const stdout = result[0].value.stdout;
