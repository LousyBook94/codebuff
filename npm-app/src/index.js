...
import { getProjectRoot, getWorkingDirectory, initializeProjectRootAndWorkingDir, initProjectFileContextWithWorker, } from './project-files';
...
async function codebuff({ initialInput, git, costMode, runInitFlow, model, agent, params, print, cwd, trace, }) {
    ...
    const loadAndValidatePromise = loadLocalAgents({
        verbose: true,
    }).then((agents) => {
...
`);
    ...
    if (options.pro) {
        console.error(red('Warning: The --pro flag is deprecated. Please restart codebuff and use the --max option instead.'));
        logger.error({
