 ...
 */
export async function runFileChangeHooks(filesChanged) {
    ...
    for (const hook of config.fileChangeHooks) {
        ...
        try {
            ...
            // }
            const result = await runTerminalCommand(hookId, hook.command, 'assistant', 'SYNC', -1, hook.cwd || getProjectRoot(), undefined, undefined);
