...
import { codebuffConfigFile, codebuffConfigFileBackup, CodebuffConfigSchema, } from '@codebuff/common/json-config/constants';
...
export function loadCodebuffConfig() {
    ...
    try {
        ...
        if (!result.success) {
            console.warn(yellow(`Warning: Invalid ${codebuffConfigFile} configuration. Please check the schema:\n` +
                result.error.issues
        ...
        if (error instanceof SyntaxError) {
            console.warn(yellow(`Warning: Invalid JSON in ${codebuffConfigFile}. Please check the syntax.`));
        }
        else {
            console.warn(yellow(`Warning: Error reading ${codebuffConfigFile} configuration file.`));
        }
        return getDefaultConfig();
    }
}
