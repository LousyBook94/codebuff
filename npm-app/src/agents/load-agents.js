...
export let loadedAgents = {};
export async function loadLocalAgents({ agentsPath, verbose = false, }) {
    loadedAgents = {};
    ...
    try {
        ...
        for (const fullPath of tsFiles) {
            ...
            try {
                ...
                if (verbose) {
                    console.error(`Error importing agent: ${error.name} - ${error.message}\n${error.stack}`, fullPath);
                }
                continue;
            }
            ...
            if (!agentDefinition.id || !agentDefinition.model) {
                if (verbose) {
                    console.error('Agent definition missing required attributes (id, model):', fullPath, 'Found:', { id: agentDefinition.id, model: agentDefinition.model });
                }
                continue;
            }
...
export function getLoadedAgentNames() {
    return Object.fromEntries(Object.entries(loadedAgents).map(([agentType, agentConfig]) => {
 ...
 */
export function displayLoadedAgents(codebuffConfig) {
    ...
    if (spawnableAgents) {
        console.log(`${green('Configured spawnable agents:')} ${spawnableAgents
            .map((name) => cyan(name))
            .join(', ')}\n`);
    }
