...
export const handleUpdateFile = async (parameters, _id) => {
    const projectPath = getProjectRoot();
    const fileChange = FileChangeSchema.parse(parameters);
    const lines = fileChange.content.split('\n');
    await waitForPreviousCheckpoint();
    const { created, modified, ignored, invalid, patchFailed } = applyChanges(projectPath, [fileChange]);
        ...
        result.push([
            {
                ...
                value: {
                    file,
                    errorMessage: 'Failed to write to file: file is ignored by .gitignore or .codebuffignore',
                },
            },
        ]);
    }
    ...
    if (result.length !== 1) {
        throw new Error(`Internal error: Unexpected number of matching results for ${{ parameters }}, found ${result.length}, expected 1`);
    }
    return result[0];
};
export const handleRunTerminalCommand = async (parameters, id) => {
    const { command, mode = 'assistant', process_type = 'SYNC', cwd, timeout_seconds = 30, } = parameters;
    await waitForPreviousCheckpoint();
    if (mode === 'assistant' && process_type === 'BACKGROUND') {
        const client = Client.getInstance();
        client.oneTimeFlags[SHOULD_ASK_CONFIG] = true;
    }
    return await runTerminalCommand(id, command, mode, process_type.toUpperCase(), timeout_seconds, cwd);
};
export const handleCodeSearch = async (parameters, _id) => {
    const projectPath = getProjectRoot();
    ...
    return new Promise((resolve) => {
        ...
        console.log();
        console.log(green(`Searching ${parameters.cwd ? `${basename}/${parameters.cwd}` : basename} for "${pattern}"${flags.length > 0 ? ` with flags: ${flags.join(' ')}` : ''}:`));
        ...
        childProcess.on('close', (code) => {
            ...
            console.log(green(`Found ${limitedLines.length} results${lines.length > maxResults ? ` (limited from ${lines.length})` : ''}`));
            // Limit results to maxResults
            const limitedStdout = limitedLines.join('\n');
            // Add truncation message if results were limited
            const finalStdout = lines.length > maxResults
                ? limitedStdout +
...
const handleFileChangeHooks = async (parameters) => {
    // Wait for any pending file operations to complete
    await waitForPreviousCheckpoint();
    const { toolResults, someHooksFailed } = await runFileChangeHooks(parameters.files);
    // Add a summary if some hooks failed
    if (someHooksFailed) {
        toolResults[0].value.push({
            errorMessage: 'Some file change hooks failed. Please review the output above.',
        });
    }
    if (toolResults[0].value.length === 0) {
        toolResults[0].value.push({
            errorMessage: 'No file change hooks were triggered for the specified files.',
        });
    }
    return toolResults;
};
const handleBrowserLogs = async (params, _id) => {
