...
export function setMessages(messages) {
    ...
    const cleanedMessages = messages.map((msg, index) => {
        ...
        const cleanContent = (content) => {
            // Keep only tool logs
            content = transformJsonInString(content, 'logs', (logs) => logs.filter((log) => log.source === 'tool'), '(LOGS_REMOVED)');
            // Remove metrics
            content = transformJsonInString(content, 'metrics', () => '(METRICS_REMOVED)', '(METRICS_REMOVED)');
            return content;
        };
