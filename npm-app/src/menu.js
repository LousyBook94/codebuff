...
import { blue, blueBright, bold, cyan, gray, green, magenta, red, underline, yellow, } from 'picocolors';
import { getProjectRoot } from './project-files';
export const interactiveCommandDetails = [
    {
        baseCommand: 'help',
...
export function getSlashCommands() {
    return interactiveCommandDetails.filter((cmd) => cmd.isSlashCommand && cmd.baseCommand);
}
export function displaySlashCommandHelperMenu() {
    ...
    const shellTip = gray('Tip: Type "!" followed by a command to run it in your shell, e.g., !ls');
    // Print with consistent spacing
    console.log(`\n\n${commandLines.join('\n')}\n${shellTip}\n`);
}
export function displayGreeting(costMode, username) {
    ...
    const costModeDescription = {
        ...
        normal: '',
        max: bold(blueBright('Max mode️ ⚡ enabled (switch modes by typing in "/")')),
...
`.trim());
    ...
    console.log(`Codebuff will read and write files in "${getProjectRoot()}". Type "/help" for a list of commands.`);
    const gitDir = path.join(getProjectRoot(), '.git');
    if (!fs.existsSync(gitDir)) {
        console.info(magenta("Just fyi, this project doesn't contain a .git directory (are you at the top level of your project?). Codebuff works best with a git repo!"));
    }
...
`);
    ...
    console.log((() => {
        ...
        const hasGitIgnore = fs.existsSync(path.join(getProjectRoot(), '.gitignore'));
        const hasKnowledgeMd = fs.existsSync(path.join(getProjectRoot(), 'knowledge.md'));
        const hasCodebuffJson = fs.existsSync(path.join(getProjectRoot(), codebuffConfigFile));
...
${!hasKnowledgeMd && !hasCodebuffJson ? `\n${yellow('knowledge.md helps Codebuff understand your project structure and codebase better for better results.')}\n${yellow(`${codebuffConfigFile} runs deployment scripts for you to test your code and runs configured checks for you by running your dev server.`)}` : ''}`;
    ...
    console.log(`Thanks for using Codebuff! DM @brandonkachen or @jahooma on Discord, or email ${blueBright('founders@codebuff.com')}`);
    console.log(`Join our Discord: ${blueBright('https://codebuff.com/discord')} ${gray(`(${os.platform() === 'darwin' ? 'Command' : 'Ctrl'} + Click to open)`)}`);
