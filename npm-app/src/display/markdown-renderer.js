...
export class MarkdownStreamRenderer {
    ...
    processSmartMode(text, outs) {
        ...
        for (let i = 0; i < linesToProcess.length; i++) {
            ...
            const nextLine = i < linesToProcess.length - 1
                ? linesToProcess[i + 1]
                : remainingLookahead;
    ...
    processLine(line, nextLine, outs) {
    ...
    isBlockComplete(block, currentLine, nextLine) {
        ...
        switch (block.type) {
            ...
            case 'list':
                ...
                if (trimmedNext === '') {
                    ...
                    const hasMoreListItems = remainingLines.some((line) => line.trim().match(/^(\s*)([-*+]|\d+\.)\s/));
    ...
    processConservativeMode(text, outs) {
    ...
    render(md) {
        ...
        const padLeft = '  ';
        const padRight = '  ';
        const mdWithPlaceholders = md.replace(codeBlockRegex, (match, fence, lang, code) => {
            ...
            const reset = '\x1b[0m';
            const reapplyBg = (s) => s.replace(/\x1b\[0m/g, `${reset}${bgGray}`);
            ...
            const wrapWidth = Math.max(20, // minimum width
            Math.min(maxLineLength, availableWidth, 120));
            const wrappedLines = [];
            for (const line of lines) {
                ...
                wrapped.forEach((seg) => {
                    const visibleLen = leadingWs.length + seg.replace(/\x1b\[[^m]*m/g, '').length;
                    const padding = Math.max(0, wrapWidth - visibleLen);
                    wrappedLines.push(`${bgGray}${padLeft}${leadingWs}${seg}${' '.repeat(padding)}${padRight}${reset}`);
                });
            }
        ...
        protectionIndex = 0;
        rendered = rendered.replace(/__PROTECTED_AGENT_MESSAGE_(\d+)__/g, (_, idx) => protectedLines[+idx] ?? '');
        return rendered;
    }
}
