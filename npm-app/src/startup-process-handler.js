...
export function logAndHandleStartup() {
    ...
    const { separateCodebuffInstanceRunning, cleanUpPromise } = cleanupStoredProcesses();
    ...
    if (config?.startupProcesses) {
        if (!separateCodebuffInstanceRunning) {
            ...
            console.log(gray('Another instance of codebuff detected. Skipping startup processes.') + '\n');
        }
    }
