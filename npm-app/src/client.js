...
import { InitResponseSchema, MessageCostResponseSchema, PromptResponseSchema, UsageReponseSchema, } from '@codebuff/common/actions';
...
import { ASKED_CONFIG, CREDITS_REFERRAL_BONUS, ONE_TIME_LABELS, ONE_TIME_TAGS, REQUEST_CREDIT_SHOW_THRESHOLD, SHOULD_ASK_CONFIG, UserState, ASYNC_AGENTS_ENABLED, API_KEY_ENV_VAR, } from '@codebuff/common/old-constants';
import { getInitialSessionState } from '@codebuff/common/types/session-state';
import { getAllAgents, resolveNameToId, } from '@codebuff/common/util/agent-name-resolver';
...
import { blue, blueBright, bold, cyan, green, red, underline, yellow, } from 'picocolors';
...
import { clearCachedProjectFileContext, getFiles, getProjectFileContext, getProjectRoot, getWorkingDirectory, startNewChat, } from './project-files';
import { logAndHandleStartup } from './startup-process-handler';
import { printSubagentHeader } from './subagent-headers';
import { clearSubagentStorage, getAllSubagentIds, markSubagentInactive, storeSubagentChunk, addCreditsByAgentId, } from './subagent-storage';
...
async function sendActionAndHandleError(ws, action) {
    try {
...
export class Client {
    ...
    lastToolResults = [];
    model;
    oneTimeFlags = Object.fromEntries(ONE_TIME_LABELS.map((tag) => [tag, false]));
    isInitializing = false;
    agentNames = {};
    constructor({ websocketUrl, onWebSocketError, onWebSocketReconnect, freshPrompt, reconnectWhenNextIdle, costMode, model, }) {
        this.costMode = costMode;
        this.model = model;
        this.webSocket = new APIRealtimeClient(websocketUrl, onWebSocketError, onWebSocketReconnect);
        ...
        repoInfoPromise.then(() => logger.info({
    ...
    static getInstance(shouldThrow = true) {
        if (!Client.instance) {
            if (shouldThrow) {
    ...
    initSessionState(projectFileContext) {
    ...
    async handleAddApiKey(keyType, apiKey) {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_CODEBUFF_APP_URL}/api/api-keys`, {
                method: 'POST',
                headers: {
    ...
    async handleReferralCode(referralCode) {
        if (this.user) {
            try {
                const redeemReferralResp = await fetch(`${process.env.NEXT_PUBLIC_CODEBUFF_APP_URL}/api/referrals`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        Cookie: `next-auth.session-token=${this.user.authToken};`,
                ...
                if (redeemReferralResp.ok) {
                    console.log([
                        green(`Noice, you've earned an extra ${respJson.credits_redeemed} credits!`),
        ...
        if (this.user) {
            try {
                ...
                try {
                    ...
                    this.oneTimeFlags = Object.fromEntries(ONE_TIME_LABELS.map((tag) => [tag, false]));
                }
                catch (error) {
                    logger.error({
        ...
        try {
            ...
            const pollInterval = setInterval(async () => {
                ...
                try {
                    ...
                    const statusResponse = await fetch(`${websiteUrl}/api/auth/cli/status?fingerprintId=${fingerprintId}&fingerprintHash=${fingerprintHash}&expiresAt=${expiresAt}`);
    ...
    setupSubscriptions() {
        const onError = (action) => {
            if (action.error === 'Insufficient credits') {
                ...
                console.error(`Visit ${blue(bold(process.env.NEXT_PUBLIC_CODEBUFF_APP_URL + '/usage'))} to add credits.`);
            }
                ...
                console.error(yellow(`Visit ${blue(bold(process.env.NEXT_PUBLIC_CODEBUFF_APP_URL + '/usage'))} to update your payment settings.`));
            }
            else {
                console.error(['', red(`Error: ${action.message}`)].join('\n'));
                logger.error({
        ...
        this.webSocket.subscribe('usage-response', (action) => {
            ...
            if (!parsedAction.success) {
                console.error(red('Received invalid usage data from server:'), parsedAction.error.issues);
                logger.error({
    ...
    async sendUserInput(prompt) {
        ...
        const toolResults = buildArray(...(this.lastToolResults || []), ...getBackgroundProcessUpdates(), scrapedContent && {
            type: 'tool-result',
    ...
    parseAgentReferences(prompt) {
        ...
        const localAgentInfo = Object.fromEntries(Object.entries(localAgentNames).map(([id, displayName]) => [
            id,
            { displayName },
        ]));
        const allAgentNames = getAllAgents(localAgentInfo).map((agent) => agent.displayName);
    ...
    handleInitializationComplete() {
        ...
        console.log(cyan(`\n📋 What codebuff.json does:\n• ${bold('startupProcesses')}: Automatically runs development servers, databases, etc. when you start Codebuff\n• ${bold('fileChangeHooks')}: Runs tests, linting, and type checking when you modify files\n• ${bold('maxAgentSteps')}: Controls how many steps the AI can take before stopping\n\n💡 Tips:\n• Add your dev server command to startupProcesses to auto-start it\n• Configure fileChangeHooks to catch errors early\n• The AI will use these hooks to verify changes work correctly\n`));
    ...
    cancelCurrentInput() {
        ...
        this.nonCancelledUserInputIds = this.nonCancelledUserInputIds.filter((id) => id !== this.userInputId);
        sendActionAndHandleError(this.webSocket, {
    ...
    subscribeToResponse(onChunk, userInputId, onStreamStart, prompt, startTime) {
        ...
        const stopResponse = () => {
            ...
            const { mainAgentState: { messageHistory }, } = this.sessionState;
        ...
        unsubscribeComplete = this.webSocket.subscribe('prompt-response', async (action) => {
            ...
            askConfig: if (this.oneTimeFlags[SHOULD_ASK_CONFIG] &&
...
Go to https://www.codebuff.com/config for more information.`) +
            ...
            allSubagentIds.forEach((agentId) => {
            ...
            const credits = Object.entries(this.creditsByPromptId)
                ...
                .reduce((total, [, creditValues]) => total + creditValues.reduce((sum, current) => sum + current, 0), 0);
    ...
    async getUsage() {
        try {
            ...
            if (coverage.isCovered && coverage.organizationName) {
                ...
                console.log(green(`🏢 Your usage in this repository is covered by ${bold(coverage.organizationName)}.`));
                ...
                if (coverage.organizationSlug) {
                    ...
                    console.log(`View your organization's usage details: ${underline(blue(orgUsageLink))}`);
                }
            }
            else {
                ...
                if (this.usageData.next_quota_reset) {
                    ...
                    console.log(`Free credits will renew on ${dateDisplay}. Details: ${underline(blue(usageLink))}`);
                }
                this.showUsageWarning();
            }
        }
            ...
            console.error(red(`Error checking usage: Please reach out to ${process.env.NEXT_PUBLIC_SUPPORT_EMAIL} for help.`));
    ...
    async checkRepositoryCoverage(remoteUrl) {
        try {
            ...
            const response = await fetch(`${backendUrl}/api/orgs/is-repo-covered`, {
                method: 'POST',
                headers: addAuthHeader({ 'Content-Type': 'application/json' }, this.user.authToken),
                body: JSON.stringify({
