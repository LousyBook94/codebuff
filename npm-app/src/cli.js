...
import { API_KEY_ENV_VAR, ASYNC_AGENTS_ENABLED, } from '@codebuff/common/old-constants';
import { getAllAgents, getAgentDisplayName, } from '@codebuff/common/util/agent-name-resolver';
import { isDir } from '@codebuff/common/util/file';
import { pluralize } from '@codebuff/common/util/string';
import { uniq } from 'lodash';
import { blueBright, bold, cyan, gray, green, magenta, red, yellow, } from 'picocolors';
import { loadLocalAgents, loadedAgents } from './agents/load-agents';
import { killAllBackgroundProcesses, sendKillSignalToAllBackgroundProcesses, } from './background-process-manager';
import { checkpointManager } from './checkpoints/checkpoint-manager';
import { enterAgentsBuffer, isInAgentsMode, cleanupAgentsBuffer, } from './cli-handlers/agents';
import { detectApiKey, handleApiKeyInput } from './cli-handlers/api-key';
import { displayCheckpointMenu, handleClearCheckpoints, handleRedo, handleRestoreCheckpoint, handleUndo, isCheckpointCommand, listCheckpoints, saveCheckpoint, } from './cli-handlers/checkpoint';
...
import { cleanupSubagentListBuffer, enterSubagentListBuffer, isInSubagentListMode, resetSubagentSelectionToLast, } from './cli-handlers/subagent-list';
import { cleanupSubagentBuffer, displaySubagentList, enterSubagentBuffer, isInSubagentBufferMode, } from './cli-handlers/traces';
...
import { disableSquashNewlines, enableSquashNewlines, } from './display/squash-newlines';
import { displayGreeting, displayMenu, displaySlashCommandHelperMenu, getSlashCommands, interactiveCommandDetails, } from './menu';
import { getProjectRoot, getWorkingDirectory, initProjectFileContextWithWorker, } from './project-files';
import { rageDetectors } from './rage-detectors';
import { logAndHandleStartup } from './startup-process-handler';
import { getRecentSubagents, setTraceEnabled } from './subagent-storage';
import { clearScreen, isCommandRunning, killAndResetPersistentProcess, persistentProcess, resetShell, } from './terminal/run-command';
...
let cachedLocalAgentInfo = {};
/**
 * Get local agent names using the proper agent loading logic
 * @returns Record of agent type to agent info
 */
export async function getLocalAgentInfo() {
    try {
        await loadLocalAgents({ verbose: false });
        const agentInfo = Object.fromEntries(Object.entries(loadedAgents).map(([agentType, agentConfig]) => [
            agentType,
            {
 ...
 */
function getCachedLocalAgentInfo() {
 ...
 */
export async function validateAgent(agent, localAgents) {
    const agents = localAgents ?? {};
    // if local agents are loaded, they're already validated
    const localById = agents?.[agent];
    const localByDisplay = Object.values(agents ?? {}).find((a) => a?.displayName === agent);
...
export class CLI {
    ...
    rl;
    constructor(readyPromise, { git, costMode, model, agent, params, print, trace }) {
        this.git = git;
    ...
    calculateAgentNameCompletions(line) {
        if (!line.includes('@')) {
            return null;
        }
        ...
        const matchingAgents = allAgentNames.filter((name) => name.toLowerCase().startsWith(searchTerm));
    ...
    calculateAtFilenameCompletions(line) {
        if (!line.includes('@')) {
            return null;
        }
        const $split = line.split('@');
        ...
        const matchingPaths = uniq(allFiles
            .map((filePath) => {
            let candidate = null;
            while (filePath.includes(searchTerm)) {
                candidate = filePath;
            ...
            .filter((filePath) => !!filePath)).sort((a, b) => priority(a) - priority(b));
    ...
    inputCompleter(line) {
        ...
        try {
            ...
            const fsMatches = files
                ...
                .map((file) => file + (isDir(path.join(baseDir, file)) ? directorySuffix : ''));
    ...
    displayAgentMenu() {
        ...
        const maxNameLength = Math.max(...uniqueAgentNames.map((agent) => agent.displayName.length));
        ...
        const tip = gray('Tip: Type "@" followed by an agent name to request a specific agent\n- You can also use "@" to search for files');
        console.log(`\n\n${agentLines.join('\n')}\n${tip}\n`);
    }
    getModeIndicator() {
    ...
    async resetAgent(agent, initialParams, userPrompt) {
        const client = Client.getInstance();
        // Reset context first
        ...
        const agentDisplayName = getAgentDisplayName(agent || 'base', localAgentInfo);
    ...
    async handleUserInput(userInput) {
        ...
        rageDetectors.repeatInputDetector.recordEvent(cleanedInput.toLowerCase().trim());
        const processedResult = await withHangDetection(userInput, () => this.processCommand(userInput));
    ...
    isKnownSlashCommand(command) {
        return getSlashCommands().some((cmd) => cmd.baseCommand === command || cmd.aliases?.includes(command));
    }
    /**
     ...
     */
    isCommandOrAlias(input, baseCommand) {
        const commandInfo = interactiveCommandDetails.find((cmd) => cmd.baseCommand === baseCommand);
        return (input === baseCommand || (commandInfo?.aliases?.includes(input) ?? false));
    }
    /**
    ...
    handleUnknownCommand(command) {
        console.log(yellow(`Unknown slash command: ${command}`) +
    ...
    async processCommand(userInput) {
        ...
        const commandInfo = interactiveCommandDetails.find((cmd) => cmd.baseCommand === cleanInput || cmd.aliases?.includes(cleanInput));
        ...
        if (costModeMatch) {
            ...
            const hasSlash = userInput.startsWith('/');
            const commandInfo = interactiveCommandDetails.find((cmd) => cmd.baseCommand === mode);
            ...
            if (mode === 'lite') {
                ...
                console.log(blueBright('⚡ Switched to max mode (slower, more thorough)'));
            }
            else if (mode === 'experimental') {
                console.log(magenta('🧪 Switched to experimental mode (cutting-edge)'));
            }
            else if (mode === 'ask') {
                console.log(cyan('💬 Switched to ask mode (questions & planning only, no code changes)'));
                console.log(gray('Tip: Use /export to save conversation summary to a file after fleshing out a plan'));
            }
            if (!message) {
                this.freshPrompt();
                return null;
        ...
        if (detectionResult.status !== 'not_found') {
            await handleApiKeyInput(Client.getInstance(), detectionResult, this.readyPromise, this.freshPrompt.bind(this));
            return null;
        }
        ...
        if (cleanInput === 'reset') {
            ...
            const initFileContextPromise = initProjectFileContextWithWorker(projectRoot, true);
            this.readyPromise = Promise.all([
        ...
        if (this.isCommandOrAlias(commandBase, 'trace')) {
            ...
            if (spaceIndex > 0) {
                ...
                if (!agentId) {
                    console.log(yellow(`Please provide a trace ID. Usage: ${commandBase} <trace-id>`));
                ...
                if (isInSubagentBufferMode()) {
                    console.log(yellow('Already in trace buffer mode! Press ESC to exit.'));
                    this.freshPrompt();
                    return null;
                }
    ...
    async forwardUserInput(promptContent) {
        ...
        const { responsePromise, stopResponse } = await client.sendUserInput(cleanedInput);
            ...
            if (isAtStart || isAfterSpace) {
                ...
                setTimeout(async () => {
                    ...
                    if (!this.isPasting) {
                        ...
                        const updatedLine = this.pastedContent + this.rl.line;
    ...
    async handleExit() {
        ...
        if (coverage.isCovered && coverage.organizationName) {
            ...
            console.log(green(`Your usage in this repository was covered by the ${bold(coverage.organizationName)} organization.`));
        }
        else {
            ...
            if (client.usageData.next_quota_reset) {
                const daysUntilReset = Math.ceil((new Date(client.usageData.next_quota_reset).getTime() - Date.now()) /
                    (1000 * 60 * 60 * 24));
                console.log(`Your free credits will reset in ${pluralize(daysUntilReset, 'day')}.`);
            }
        }
