...
import { getSubagentData, getSubagentFormattedContent, getRecentSubagents, } from '../subagent-storage';
import { enterSubagentListBuffer } from './subagent-list';
import { ENTER_ALT_BUFFER, EXIT_ALT_BUFFER, CLEAR_SCREEN, HIDE_CURSOR, SHOW_CURSOR, MOVE_CURSOR, } from '../utils/terminal';
/**
 * Wrap a line to fit within terminal width using robust npm packages
 */
function wrapLine(line, terminalWidth) {
    if (!line)
        return [''];
    ...
    if (agents.length === 0) {
        ...
        agents.forEach((agent) => {
            ...
            const promptPreview = agent.prompt
                ? gray(agent.prompt)
                : gray('(no prompt)');
            console.log(`  ${status} ${bold(agent.agentId)} ${gray(`(${agent.agentType})`)}`);
            console.log(`    ${promptPreview}`);
            console.log();
        });
    }
}
export function enterSubagentBuffer(rl, agentId, onExit) {
...
export function exitSubagentBuffer(rl) {
    if (!isInSubagentBuffer) {
        return;
    }
    // Reset state
...
function setupSubagentKeyHandler(rl, onExit) {
 ...
 */
export function refreshSubagentDisplay(agentId) {
