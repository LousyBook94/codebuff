...
let scrollOffset = 0;
let allContentLines = [];
let subagentLinePositions = [];
let subagentList = [];
export function isInSubagentListMode() {
    return isInSubagentListBuffer;
}
export function enterSubagentListBuffer(rl, onExit) {
    ...
    if (subagentList.length === 0) {
        ...
        console.log(gray('Traces will appear here after you use spawn_agents in a conversation.'));
...
export function exitSubagentListBuffer(rl) {
    if (!isInSubagentListBuffer) {
        return;
    }
    ...
    if (subagentList.length === 0) {
        ...
        for (let i = 0; i < subagentList.length; i++) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (inCodeBlock || line.trim().startsWith('```')) {
                        ...
                        for (const word of words) {
                            ...
                            if (testLine.replace(/\u001b\[[0-9;]*m/g, '').length <=
                                maxLineLength) {
            ...
            const timeInfo = agent.isActive
                ...
                : gray(`[${startTime}]`);
            const creditsDisplay = agent.creditsUsed > 0
...
function setupSubagentListKeyHandler(rl, onExit) {
    ...
    process.stdin.on('keypress', (str, key) => {
        ...
        if (key && key.name === 'pagedown') {
            const newIndex = Math.min(subagentList.length - 1, persistentSelectedIndex + 5); // Jump 5 items down
            if (newIndex !== persistentSelectedIndex) {
                persistentSelectedIndex = newIndex;
                centerSelectedItem();
                renderSubagentList();
