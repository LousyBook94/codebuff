 ...
 */ export async function handlePublish(agentIds) {
    ...
    if (agentIds?.length === 0) {
        console.log(red('Agent id is required. Usage: publish <agent-id> [agent-id2] ...'));
    ...
    try {
        ...
        for (const agentId of agentIds) {
            ...
            const matchingTemplate = Object.values(agentTemplates).find((template) => template.id === agentId || template.displayName === agentId);
            if (!matchingTemplate) {
        ...
        try {
            const result = await publishAgentTemplates(Object.values(matchingTemplates), user.authToken);
            if (result.success) {
                console.log(green(`✅ Successfully published:`));
                for (const agent of result.agents) {
                    console.log(cyan(`  - ${agent.displayName} (${result.publisherId}/${agent.id}@${agent.version})`));
                }
                return;
            }
            ...
            if (result.error?.includes('Publisher field required')) {
                ...
                console.log();
                console.log(cyan('Check that the publisher ID is correct and you have access to it.'));
                console.log();
            }
            ...
            console.log(red(`❌ Error publishing agents: ${error instanceof Error ? error.message : String(error)}`));
        ...
        console.log(red(`Error during publish: ${error instanceof Error ? error.message + '\n' + error.stack : String(error)}`));
        // Avoid logger.error here as it can cause sonic boom errors that mask the real error
        // The error is already displayed to the user via console.log above
    }
}
 ...
 */
async function publishAgentTemplates(data, authToken) {
    try {
