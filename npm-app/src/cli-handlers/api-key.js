 ...
 */
export function detectApiKey(userInput) {
 ...
 */
export async function handleApiKeyInput(client, detectionResult, readyPromise, returnControlToUser) {
    switch (detectionResult.status) {
        ...
        case 'prefix_only':
            ...
            console.log(yellow(`Input looks like a ${detectionResult.type} API key but has the wrong length or format. Expected ${detectionResult.length} characters starting with "${detectionResult.prefix}".`));
