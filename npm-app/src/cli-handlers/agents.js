...
import { getLoadedAgentNames, loadedAgents, } from '../agents/load-agents';
...
import { ENTER_ALT_BUFFER, EXIT_ALT_BUFFER, CLEAR_SCREEN, HIDE_CURSOR, SHOW_CURSOR, } from '../utils/terminal';
let isInAgentsBuffer = false;
let originalKeyHandlers = [];
let selectedIndex = 0;
let scrollOffset = 0;
let allContentLines = [];
let agentLinePositions = [];
let agentList = [];
export function isInAgentsMode() {
    return isInAgentsBuffer;
}
export async function enterAgentsBuffer(rl, onExit) {
    if (isInAgentsBuffer) {
    ...
    const createAgentDescription = (entry) => {
    ...
    if (validAgents.length > 0) {
        if (recentAgents.length > 0) {
            ...
            for (const entry of recentAgents) {
                const agentName = localAgents[entry.agentId] || entry.def?.displayName || entry.agentId;
    ...
    while (selectedIndex < agentList.length &&
...
export function exitAgentsBuffer(rl) {
    if (!isInAgentsBuffer) {
        return;
    }
    // Reset state
...
function centerSelectedItem() {
    ...
    const maxScrollOffset = Math.max(0, allContentLines.length - maxScrollableLines);
...
const getHeaderLines = (terminalWidth) => [
        ...
        for (let i = 0; i < agentList.length; i++) {
            ...
            if (agent.isSectionHeader) {
                ...
                if (isSelected) {
                    ...
                    lines.push(`  ${cyan('│')} ${gray(separatorLine)}  ${agent.name} ${cyan('│')}`);
                    if (agent.description) {
                        const descPadding = Math.max(0, headerWidth - cleanDescription.length);
                        lines.push(`  ${cyan('│')} ${agent.description}${' '.repeat(descPadding)} ${cyan('│')}`);
                    }
                    lines.push(`  ${cyan('└' + '─'.repeat(headerWidth + 2) + '┘')}`);
                }
                else {
                    // Right-aligned title with separator line for unselected
                    const titlePadding = Math.max(0, availableWidth - cleanName.length - 4);
            ...
            if (agent.isPlaceholder) {
                if (isSelected) {
                    ...
                    lines.push(`  ${cyan('│')} ${agent.name} ${' '.repeat(Math.max(0, boxWidth - agent.name.replace(/\u001b\[[0-9;]*m/g, '').length))} ${cyan('│')}`);
                    lines.push(`  ${cyan('│')} ${gray(agent.description || '')} ${' '.repeat(Math.max(0, boxWidth - (agent.description || '').length))} ${cyan('│')}`);
                    lines.push(`  ${cyan('└' + '─'.repeat(boxWidth + 2) + '┘')}`);
                }
                else {
            ...
            // Regular agent items
            const agentInfo = agent.isCreateNew || agent.isEditAgent
            ...
            if (isSelected) {
                ...
                const maxContentWidth = Math.max(...contentForBox.map((line) => line.replace(/\u001b\[[0-9;]*m/g, '').length));
...
function renderAgentsList() {
    ...
    const visibleLines = allContentLines.slice(scrollOffset, scrollOffset + maxScrollableLines);
...
function setupAgentsKeyHandler(rl, onExit) {
    // Store all original key handlers
    const listeners = process.stdin.listeners('keypress');
    originalKeyHandlers = listeners;
    ...
    process.stdin.on('keypress', (str, key) => {
        ...
        if (key && (key.name === 'down' || key.name === 'j')) {
            let newIndex = selectedIndex + 1;
            while (newIndex < agentList.length &&
                (agentList[newIndex]?.isSeparator ||
                    agentList[newIndex]?.isPlaceholder ||
                    agentList[newIndex]?.isSectionHeader)) {
                newIndex++;
...
async function startDirectAgentCreation(onExit) {
    try {
        await createExampleAgentFiles();
        console.log(green('\n✅ Created example agent files in .agents directory!'));
        console.log(gray('Check out the files and edit them to create your custom agents.'));
        console.log(gray('Run "codebuff --agent your-agent-id" to test your agents.'));
    }
    catch (error) {
