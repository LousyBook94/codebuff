...
import { stripColors, truncateStringWithMessage, } from '@codebuff/common/util/string';
import { green } from 'picocolors';
import { getProjectRoot, getWorkingDirectory, setWorkingDirectory, } from '../project-files';
...
function createWrapperScript(shell, initLines, userCmd) {
...
function buildWinInvocation(shell, initLines, userCmd) {
...
async function createPersistentProcess(dir) {
...
/* ------------------------------------------------------------------ */
export const runTerminalCommand = async (toolCallId, command, mode, processType, timeoutSeconds, cwd, stdoutFile, stderrFile) => {
    ...
    if (processType === 'BACKGROUND') {
        return new Promise((res) => runBackgroundCommand({ toolCallId, command: modifiedCmd, mode, cwd, stdoutFile, stderrFile }, (v) => {
            res(resolveCommand(v));
        }));
    }
    /* sync mode --------------------------------------------------------- */
    return new Promise((resolve) => runCommandChildProcess(persistentProcess, modifiedCmd, mode, cwd, maybeTimeoutSeconds, (v) => resolve(resolveCommand(v))));
};
/* ------------------------------------------------------------------ */
/* core command runner                                                */
/* ------------------------------------------------------------------ */
const runCommandChildProcess = async (pp, command, mode, cwd, maybeTimeoutSeconds, resolve) => {
    const projectRoot = getProjectRoot();
    /* clear screen ----------------------------------------------------- */
    ...
    if (mode === 'assistant') {
        const displayDir = path.join(path.parse(projectRoot).base, path.relative(projectRoot, path.resolve(projectRoot, cwd)));
    ...
    let child;
    if (IS_WINDOWS) {
        const { exe, args } = buildWinInvocation(shell, initLines, command);
        child = spawn(exe, args, { cwd, env: pp.env });
    }
    else {
        const script = createWrapperScript(shell, initLines, command);
