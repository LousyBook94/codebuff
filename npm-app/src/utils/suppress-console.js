...
const activeSuppressFunctions = {};
// Initialize original methods and active functions
function initializeConsoleMethod(type) {
...
function createWrappedConsoleMethod(type) {
    return function (...args) {
        ...
        const shouldSuppress = activeSuppressFunctions[type].some((suppressFn) => suppressFn(args, errorName));
        if (shouldSuppress) {
...
export function suppressConsoleOutput(type, suppress) {
    if (type === 'all') {
        // Apply to all console methods
        const cleanupFunctions = ['error', 'warn', 'log'].map((consoleType) => suppressConsoleOutput(consoleType, suppress));
