...
function suppressPostHogConsoleErrors() {
    suppressConsoleOutput('error', (args, errorName) => errorName.toLowerCase().includes('posthog'));
    suppressConsoleOutput('warn', (args, errorName) => errorName.toLowerCase().includes('posthog'));
}
export function initAnalytics() {
    ...
    client = new PostHog(process.env.NEXT_PUBLIC_POSTHOG_API_KEY, {
        ...
        enableExceptionAutocapture: process.env.NEXT_PUBLIC_CB_ENVIRONMENT === 'prod',
    });
...
export function logError(error, userId, properties) {
    if (!client) {
        return;
    }
    try {
        client.captureException(error, userId ?? currentUserId ?? 'unknown', properties);
    }
    catch (postHogError) {
