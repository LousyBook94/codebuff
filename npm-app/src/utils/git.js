...
export function gitCommandIsAvailable() {
...
export async function getRepoMetrics(providedRemoteUrl) {
    const t = Date.now();
    ...
    const commitsArr = await log({ fs, dir: root, gitdir: gitDir }).catch((error) => {
        logger.error({ error }, 'Error fetching git log. Is this an empty git repo?');
        return [];
    });
