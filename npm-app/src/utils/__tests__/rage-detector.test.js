...
import { createCountDetector, createTimeBetweenDetector, createTimeoutDetector, } from '../rage-detector';
describe('Rage Detectors', () => {
    ...
    beforeEach(() => {
        ...
        // Mock setTimeout
        mockSetTimeout = spyOn(global, 'setTimeout').mockImplementation(((callback, delay) => {
            const id = timeoutId++;
            timeouts.push({
                callback,
                delay,
        ...
        // Mock clearTimeout
        mockClearTimeout = spyOn(global, 'clearTimeout').mockImplementation(((id) => {
    ...
    describe('createTimeoutDetector', () => {
        test('should fire when stop() is not called within timeout', () => {
            ...
            expect(mockTrackEvent).toHaveBeenCalledWith(AnalyticsEvent.RAGE, expect.objectContaining({
                reason: 'websocket_hang',
                durationMs: 60000,
                timeoutMs: 60000,
                context: 'test',
