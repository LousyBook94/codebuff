...
function sendAnalyticsAndLog(level, data, msg, ...args) {
    if (process.env.CODEBUFF_GITHUB_ACTIONS !== 'true' &&
        ...
        const logTarget = process.env.NEXT_PUBLIC_CB_ENVIRONMENT === 'dev'
...
function logAsErrorIfNeeded(toTrack) {
    if (toTrack.level === 'error' || toTrack.level === 'fatal') {
        logError(new Error(toTrack.msg), toTrack.loggerContext.userId ?? 'unknown', { ...toTrack.data, context: toTrack.loggerContext });
        flushAnalytics();
    }
}
/**
 ...
 */
export const logger = Object.fromEntries(loggingLevels.map((level) => {
    return [
        level,
