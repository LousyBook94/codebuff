 ...
 */
const renderSpawnAgentsParam = (paramName, toolName, content) => {
    if (paramName === 'agents') {
        let agents = [];
        try {
        ...
        if (agents.length > 0) {
            ...
            return gray(agents
                ...
                const agentName = (client?.agentNames && client.agentNames[agentType]) || agentType;
 ...
 */
export const defaultToolCallRenderer = {
...
export const toolRenderers = {
    ...
    read_files: {
        ...
        onParamEnd: (paramName, toolName, content) => {
            ...
            files = files.map((fname) => isFileIgnored(fname, getProjectRoot())
            ...
            if (numFiles <= maxInitialFiles) {
                ...
                return gray(`${initialFiles.map((file) => '- ' + file).join('\n')}\nand ${numRemaining} more: ${remainingFilesString}`);
            }
        },
