...
import { codebuffConfigFile, codebuffConfigFileBackup, } from '@codebuff/common/json-config/constants';
import { AGENT_TEMPLATES_DIR, FILE_READ_STATUS, toOptionalFile, } from '@codebuff/common/old-constants';
import { flattenTree, getProjectFileTree, parseGitignore, } from '@codebuff/common/project-file-tree';
...
function initializeChatId(providedChatId) {
...
export function setChatIdFromExternal(chatId) {
    if (currentChatId === null) {
...
let projectRoot;
export function setProjectRoot(dir, setCwd = false) {
    if (existsSync(dir)) {
        if (projectRoot && projectRoot !== dir) {
            checkpointManager.clearCheckpoints(true);
            console.log('\n' + green('Directory change:'), `Codebuff will read and write files in "${dir}".\n`);
        }
        projectRoot = dir;
        if (setCwd) {
...
export function getStartingDirectory(dir = undefined) {
    let base;
    try {
        base = process.cwd();
    }
 ...
 */
export function initializeProjectRootAndWorkingDir(cwd) {
 ...
 */
export function toAbsolutePath(filepath, projectRoot) {
...
export function initProjectFileContextWithWorker(dir, resetCache = false) {
    if (resetCache) {
        cachedProjectFileContext = undefined;
    }
    const workerRelativePath = './workers/project-context.ts';
    const worker = new Worker(process.env.IS_BINARY
 ...
 */
export const getProjectFileContext = async (projectRoot, lastFileVersion) => {
    ...
    if (!cachedProjectFileContext ||
        ...
        const filteredKnowledgeFilePaths = knowledgeFilePaths.filter((filePath) => !filePath.startsWith(AGENT_TEMPLATES_DIR));
        const knowledgeFiles = getExistingFiles(filteredKnowledgeFilePaths);
        const knowledgeFilesWithScrapedContent = await addScrapedContentToFiles(Object.fromEntries(Object.entries(knowledgeFiles).filter(([filePath]) => filePath !== codebuffConfigFile.toLowerCase() &&
 ...
 */
export function getChangesSinceLastFileVersion(lastFileVersion) {
...
export function getFiles(filePaths) {
    const result = {};
...
export function getFilesOrNull(filePaths) {
...
export function getExistingFiles(filePaths) {
    return filterObject(getFilesOrNull(filePaths), (value) => value !== null);
}
export async function addScrapedContentToFiles(files) {
...
export function getFilesAbsolutePath(filePaths, options = {}) {
    const result = {};
    for (const filePath of filePaths) {
        try {
