...
export function printSubagentHeader(event) {
    ...
    const dashesLength = Math.max(0, Math.floor((width - fullAgentName.length - 2) / 2));
    ...
    if (event.type === 'subagent_start') {
        ...
        const dashesLength = Math.max(0, Math.floor((width - endedFullAgentName.length - 2) / 2));
