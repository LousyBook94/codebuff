 ...
 */
export function getBareRepoPath(dir) {
    const bareRepoName = createHash('sha256').update(dir).digest('hex');
    return join(getProjectDataDir(), bareRepoName);
}
let nestedRepos = [];
function exposeSubmodules({ bareRepoPath, projectDir, }) {
    while (true) {
        const submodulesOutput = execFileSync('git', [
            '-c',
            ...
            ':(exclude)**/*.codebuffbackup/**',
        ], { stdio: ['ignore', 'pipe', 'inherit'], maxBuffer }).toString();
        const submodules = buildArray(submodulesOutput
            .split('\n')
        ...
        for (const submodule of submodules) {
            try {
                ...
                fs.renameSync(path.join(projectDir, submodule, '.git'), path.join(projectDir, submodule, '.git.codebuffbackup'));
        ...
        try {
            ...
            logger.error({ error }, 'Error running git rm --cached while exposing submodules');
        }
    }
}
function restoreSubmodules({ projectDir }) {
    for (const nestedRepo of nestedRepos) {
        const codebuffBackup = path.join(projectDir, nestedRepo, '.git.codebuffbackup');
        const gitDir = path.join(projectDir, nestedRepo, '.git');
        try {
            fs.renameSync(codebuffBackup, gitDir);
        }
        catch (error) {
            console.error(`Failed to restore .git directory for nested repo. Please rename ${codebuffBackup} to ${gitDir}\n${JSON.stringify({ error }, null, 2)}`);
            logger.error({
                error,
 ...
 */
export async function hasUnsavedChanges({ projectDir, bareRepoPath, relativeFilepaths, }) {
    ...
    try {
        ...
        try {
            const output = execFileSync('git', [
                ...
                ':(exclude)**/*.codebuffbackup',
                ':(exclude)**/*.codebuffbackup/**',
            ], { stdio: ['ignore', 'pipe', 'ignore'], maxBuffer }).toString();
            return !!output;
        }
        catch (error) {
            logger.error({
 ...
 */
export async function getLatestCommit({ bareRepoPath, }) {
    if (gitCommandIsAvailable()) {
        try {
            return execFileSync('git', ['--git-dir', bareRepoPath, 'rev-parse', 'HEAD'], { stdio: ['ignore', 'pipe', 'ignore'], maxBuffer })
                .toString()
                .trim();
        }
        catch (error) {
 ...
 */
export async function initializeCheckpointFileManager({ projectDir, relativeFilepaths, }) {
...
async function gitAddAllIgnoringNestedRepos({ projectDir, bareRepoPath, relativeFilepaths, }) {
    const nestedRepos = [];
    try {
        exposeSubmodules({ bareRepoPath, projectDir });
        let output;
        try {
            ...
            logger.error({ error, projectDir, bareRepoPath }, 'Failed to get git status while finding nested git repos');
            return;
        }
        if (!output) {
            return;
 ...
 */
export async function storeFileState({ projectDir, bareRepoPath, message, relativeFilepaths: relativeFilepaths, }) {
 ...
 */
export async function restoreFileState({ projectDir, bareRepoPath, commit, relativeFilepaths, }) {
    let resetDone = false;
    if (gitCommandIsAvailable()) {
        try {
            execFileSync('git', [
    ...
    await Promise.all(relativeFilepaths.map((filepath) => resetIndex({
        fs,
        dir: projectDir,
        gitdir: bareRepoPath,
        filepath,
