...
const subagentStorage = new Map();
// Global trace flag
let traceEnabled = false;
// Track which agents have had their headers written
const agentHeadersWritten = new Set();
/**
...
export function setTraceEnabled(enabled) {
    ...
    if (enabled) {
        ...
        try {
            ...
            logger.warn({ error: error instanceof Error ? error.message : String(error) }, 'Failed clearing traces directory');
        }
    }
}
/**
 ...
 */
function writeToTraceLog(agentId, agentType, chunk) {
    if (!traceEnabled)
        return;
    try {
        const projectRoot = getProjectRoot();
 ...
 */
export function storeSubagentChunk({ agentId, agentType, chunk, prompt, }) {
    const now = Date.now();
 ...
 */
export function addCreditsByAgentId(agentId, credits) {
    const data = subagentStorage.get(agentId);
    if (data) {
 ...
 */
export function getSubagentMessages(agentId) {
    const data = subagentStorage.get(agentId);
 ...
 */
export function getSubagentData(agentId) {
 ...
 */
export function markSubagentInactive(agentId) {
    const data = subagentStorage.get(agentId);
    if (data) {
        data.isActive = false;
    }
 ...
 */
export function getSubagentFullContent(agentId) {
 ...
 */
export function getSubagentFormattedContent(agentId) {
 ...
 */
export function getRecentSubagents(limit = 10) {
 ...
 */
export function getSubagentsChronological(limit = 50) {
    return Array.from(subagentStorage.values())
