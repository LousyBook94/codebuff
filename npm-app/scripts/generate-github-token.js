...
const PRIVATE_KEY_CONTENT = process.env.GH_APP_PRIVATE_KEY;
...
function generateJWT() {
    ...
    const payloadB64 = base64urlEscape(Buffer.from(payloadJson).toString('base64'));
    ...
    try {
        ...
        const signature = execSync(`cat "${tempFile}" | openssl dgst -sha256 -sign "${tempKeyFile}" -binary | base64 -w 0`, { encoding: 'utf8' });
...
function getInstallationToken(jwt) {
    ...
    try {
        console.log(`🔍 Getting installation token for installation ID: ${INSTALLATION_ID}`);
      ...
      https://api.github.com/app/installations/${INSTALLATION_ID}/access_tokens`, { encoding: 'utf8' });
        // Clean up temp file
...
function main() {
    ...
    console.log('');
    console.log('Copy and run the export command above, then run your release script.');
}
main();
