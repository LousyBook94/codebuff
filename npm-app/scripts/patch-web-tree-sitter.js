...
function patchSingleFile(webTreeSitterPath, verbose) {
    if (!fs.existsSync(webTreeSitterPath)) {
        if (verbose) {
            console.warn(`⚠️  web-tree-sitter not found at ${webTreeSitterPath}, skipping`);
        }
        return false;
    }
    try {
        ...
        const wasmPath = path.join(__dirname, '../../node_modules/web-tree-sitter/tree-sitter.wasm');
...
`;
        ...
        const readPattern = 'var ret = fs.readFileSync(filename, binary2 ? void 0 : "utf8");';
      ...
      if (file == wasmBinaryFile && wasmBinary) {
        ...
        const newContent2 = content.replace(getBinarySyncPattern, getBinarySyncReplacement);
        replacements.push({
        ...
        if (content !== originalContent) {
            ...
            if (verbose) {
                ...
                if (failedReplacements.length > 0) {
                    console.warn(`⚠️  Some patches failed: ${failedReplacements.map((r) => r.name).join(', ')}`);
                }
            }
            return true;
        }
            ...
            if (verbose) {
                console.log('⚠️  No changes made - all patterns may have failed to match');
                console.log(`Failed replacements: ${failedReplacements.map((r) => r.name).join(', ')}`);
            }
            return false;
        }
    }
...
export function patchWebTreeSitter(verbose = false) {
    ...
    const webTreeSitterPath = path.join(__dirname, '../../node_modules/web-tree-sitter/tree-sitter.js');
    let patchedCount = 0;
